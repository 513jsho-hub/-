...
function render() {
  ...
  const openBooths = currentData.booths.filter(b => b.isOpen);
...
async function reserve(boothId, slotId, name, classInfo) {
  closeModal();
  try {
    // 저장 직전 최신 데이터를 다시 받아와 다른 사람의 예약과 겹치지 않게 확인
    currentData = await Store.load();
    render();

    const booth = Store.findBooth(currentData, boothId);
    const slot = booth && Store.findSlot(booth, slotId);
    if (!slot || slot.reservations.length >= slot.capacity) {
