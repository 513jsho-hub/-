...
function renderDashboard(main) {
  ...
  main.innerHTML = `
    ...
    <div class="section-title">부스 목록 (${data.booths.length})</div>
  ...
  document.getElementById('changePwBtn').addEventListener('click', () => {
    ...
    const next = prompt('새 관리자 비밀번호를 입력하세요', cur.adminPassword);
    if (next && next.trim()) {
      cur.adminPassword = next.trim();
      Store.save(cur);
...
function renderBoothManageList(data) {
  ...
  el.innerHTML = data.booths.map(booth => {
...
function deleteBooth(boothId) {
  ...
  const data = Store.load();
  data.booths = data.booths.filter(b => b.id !== boothId);
  Store.save(data);
  render();
  showToast('부스가 삭제되었습니다');
}

function cancelReservation(boothId, slotId, resId) {
